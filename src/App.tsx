...
const parseNum = (x: unknown): number | undefined => {
...
const fmtYrs = (n?: number) => (n == null ? "—" : `${n.toFixed(2)} yrs`);
...
const App: React.FC = () => {
  ...
  const [ytmMin, setYtmMin] = React.useState<number | undefined>();
  const [ytmMax, setYtmMax] = React.useState<number | undefined>();
  const [durMin, setDurMin] = React.useState<number | undefined>();
  const [durMax, setDurMax] = React.useState<number | undefined>();
  const [oasMin, setOasMin] = React.useState<number | undefined>();
  const [oasMax, setOasMax] = React.useState<number | undefined>();
  ...
  // Charts
  const [tab, setTab] = React.useState<"scatter" | "ytm" | "duration">("scatter");
  const isDark = useIsDark();
  ...
  function makeBins(values: number[], step: number) {
    ...
    for (let x = min; x <= max; x += step) {
      const next = x + step;
      bins.push({
        bucket: `${x.toFixed(step < 1 ? 1 : 0)}–${next.toFixed(step < 1 ? 1 : 0)}`,
          ...
          <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <p className="text-white/90 text-sm">
                Explore fixed income ETFs: search, filter, visualize and export the
        ...
        {!loading && error && (
          <Card className="rounded-2xl shadow-sm bg-white dark:bg-slate-800">
            <CardContent className="p-6 space-y-3">
              <div className="text-red-600 font-medium">Load error</div>
                    ...
                    <ScatterChart margin={{ top: 10, right: 20, left: 0, bottom: 20 }}>
                      ...
                      <Scatter
                        ...
                        data={filteredFunds
                          .filter(
                            (f) =>
                              f["Average Yield to Maturity"] != null &&
                              f["Effective Duration"] != null
                          )
                          .map((f) => ({
